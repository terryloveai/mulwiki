...
function parseSections(config: string): { title: string; content: string }[] {
...
function sectionSummary(section: { title: string; content: string }): string {
  // Grab first non-empty, non-heading line
  ...
  const listItems = lines.filter((l) => l.trim().startsWith("-")).length;
...
function ValidationPanel({ result }: { result: ValidateSchemaResponse }) {
  if (result.valid && (!result.warnings || result.warnings.length === 0)) return null;
  return (
    <div className="mt-3 space-y-1.5">
      {result.errors?.map((e, i) => (
        <div key={`err-${i}`} className="flex items-start gap-1.5 text-xs text-destructive">
          <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
          {e}
        </div>
      ))}
      {result.warnings?.map((w, i) => (
        <div key={`warn-${i}`} className="flex items-start gap-1.5 text-xs text-warning">
          <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
          {w}
        </div>
      ))}
